...
public class BufferedSource implements Runnable, RSBufferedSource {
	...
	boolean method5907(int var1) throws IOException {
		if (var1 == 0) {
			return true;
	...
	int method5908() throws IOException {
		synchronized(this) {
			int var2;
			if (this.position <= this.limit) {
	...
	int method5909() throws IOException {
		synchronized(this) {
			if (this.position == this.limit) {
				if (this.exception != null) {
					throw new IOException(this.exception.toString());
	...
	int method5911(byte[] var1, int var2, int var3) throws IOException {
